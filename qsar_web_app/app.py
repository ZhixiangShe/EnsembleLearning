...
with col_right:
    if assets is None:
        st.error(
            f"Model file 'model_assets_{dataset_choice}.pkl' not found. Please upload it to 'deploy_models' folder.")
    else:
                ...
                with st.spinner("Calculating ECFP Atomic Contributions..."):
                    try:
                        ...
                        if 'ecfp_surrogate' not in assets:
                            st.error(
                                "ECFP Surrogate model not found in the .pkl file. Please ensure you ran the surrogate training step locally.")
                        else:
